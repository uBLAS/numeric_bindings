...
inline void unmtr( char side, UpLo, Trans, fortran_int_t m, fortran_int_t n,
...
struct unmtr_impl {
    ...
    static void invoke( const char side, const MatrixA& a,
        ...
        BOOST_ASSERT( stride_major(c) >= std::max< std::ptrdiff_t >(1,
                size_row(c)) );
        detail::unmtr( side, uplo(), trans(), size_row(c), size_column(c),
...
template< typename MatrixA, typename VectorTAU, typename MatrixC,
        typename Workspace >
inline std::ptrdiff_t unmtr( const char side, const MatrixA& a,
        const VectorTAU& tau, MatrixC& c, Workspace work ) {
    fortran_int_t info(0);
    unmtr_impl< typename value< MatrixA >::type >::invoke( side, a, tau,
            c, info, work );
    return info;
}
